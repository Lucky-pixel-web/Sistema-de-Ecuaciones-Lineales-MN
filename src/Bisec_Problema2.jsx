...
function C1(t) { return 45 + 12 * t }
function C2(t) { return 20 * Math.exp(0.4 * t) }
function f(t) { return C1(t) - C2(t) }
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={cardTransition}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <label className="text-sm text-slate-400">a (años)
        ...
        <label className="text-sm text-slate-400">b (años)
...
function ContextSolution({ a, b }) {
  ...
  return (
    <>
      <div><h3 className="text-fuchsia-400 font-semibold mb-1">Dato</h3><p>f(t) = 45 + 12t − 20·e^(0.4t) = 0, t ≥ 0 (años)</p></div>
      <div>
...
function GraphSolution({ raiz, a, b }) {
  return (
    <>
      ...
      <div><h3 className="text-fuchsia-400 font-semibold mb-1">Lectura</h3><p>{raiz ? `Punto de equilibrio en t ≈ ${raiz.xr.toFixed(4)} años.` : 'No converge: revisa que f(a) y f(b) tengan signos opuestos.'}</p></div>
      <div>
        <h3 className="text-fuchsia-400 font-semibold mb-1">Verificación en GeoGebra</h3>
        <p className="text-cyan-300 pl-4">f(x) = Función(45 + 12x - 20*ℯ^(0.4x), {a}, {b})</p>
      </div>
    </>
  )
}
...
function InterpretationSolution({ raiz, rows }) {
  ...
  return (
    <>
      <p>Tras {rows.length} iteraciones, t ≈ {raiz.xr.toFixed(4)} años, error {raiz.error?.toFixed(4)}%.</p>
      <p>Antes de ese momento la nube es más económica; después el costo exponencial la supera y conviene el sistema local.</p>
    </>
  )
}
...
export default function Problema2() {
  ...
  const domainError = (aStr === '' || bStr === '' || tolStr === '') ? 'Completa los tres valores.'
    ...
    : a < 0 ? 'a debe ser ≥ 0 (t representa años).' : (a >= b ? 'a debe ser menor que b.' : null)
  const noRootError = !domainError && f(a) * f(b) > 0
    ? 'f(a) y f(b) tienen el mismo signo: no hay garantía de raíz aquí. La raíz real está en t ≈ 3.77 años — prueba, por ejemplo, [3, 4] o [0, 5].'
    : null
  ...
  const chartData = useMemo(() => {
    const safeA = Number.isNaN(a) ? 3 : a
    const safeB = Number.isNaN(b) ? 4 : b
  ...
  return (
    <div className="relative text-slate-100 p-6 md:p-10">
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="max-w-[1400px] mx-auto relative z-10">
        <h1 className="text-3xl md:text-4xl font-bold bg-gradient-to-r from-cyan-400 to-blue-500 bg-clip-text text-transparent">Problema 2 — Método de Falsa Posición (Regula Falsi)</h1>
        <p className="text-slate-400 mt-2">Punto de equilibrio en la migración de un sistema a la nube</p>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-6 backdrop-blur-md bg-white/5 border border-white/10 rounded-2xl p-6">
          <div className="flex justify-between items-start">
            <p className="text-slate-300">f(t) = 45 + 12t − 20·e^(0.4t) = 0, t ≥ 0 (años)</p>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mt-6 mb-10 backdrop-blur-md bg-gradient-to-r from-emerald-500/10 to-cyan-500/10 border border-emerald-400/20 rounded-2xl p-6">
          ...
          {raiz ? (
            <p className="text-slate-300">Tras {rows.length} iteraciones, t ≈ <span className="font-bold text-emerald-300">{raiz.xr.toFixed(4)} años</span>, error {raiz.error?.toFixed(4)}%. Antes de ese momento la nube es más barata; después conviene lo local.</p>
