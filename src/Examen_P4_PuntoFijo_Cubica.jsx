...
// f(x) = x^3 - 3x + 1 = 0  <=>  x = g(x) = (x^3 + 1) / 3
// g(x): forma iterativa despejada de f(x)=0, usada para la sucesión x_(n+1)=g(x_n)
function g(T) { return (Math.pow(T, 3) + 1) / 3 }
// f(x): función original del problema (para verificar la solución al final)
function f(T) { return Math.pow(T, 3) - 3 * T + 1 }
// gPrime(x): derivada de g(x); si |g'(x)|<1 cerca de la raíz, el método converge
function gPrime(T) { return T * T }
...
function ContextSolution({ T0 }) {
  ...
  return (
    <>
      <div><h3 className="text-fuchsia-400 font-semibold mb-1">Dato</h3><p>x = g(x) = (x³ + 1) / 3, forma iterativa directa de f(x) = x³ − 3x + 1 = 0 (se despeja el término −3x y se divide entre −3, invirtiendo signo).</p></div>
      <div>
        <h3 className="text-fuchsia-400 font-semibold mb-1">Condición de convergencia</h3>
        <p>g'(x) = x². En x₀={T0}: g'({T0}) = {gp.toFixed(4)} → |g'|={Math.abs(gp).toFixed(4)} {Math.abs(gp) < 1 ? '< 1 → converge' : '≥ 1 → NO converge, prueba otro x₀ más cercano a la raíz ≈ 0.34730'}.</p>
      </div>
      <div><h3 className="text-fuchsia-400 font-semibold mb-1">Aproximación inicial</h3><p>Se parte de x₀={T0}, suficientemente cercano a la raíz real ≈ 0.34730 (una de las tres raíces reales de x³−3x+1=0, junto a ≈ −1.87939 y ≈ 1.53209) para que |g'(x₀)| &lt; 1.</p></div>
    </>
  )
}
...
function GraphSolution({ raiz }) {
  return (
    <>
      ...
      <div><h3 className="text-fuchsia-400 font-semibold mb-1">Lectura</h3><p>{raiz ? `Converge a x ≈ ${raiz.gTn.toFixed(5)}.` : 'No converge con estos valores.'}</p></div>
    </>
  )
}
  ...
  return (
    <>
      <div>
        ...
        <p className="bg-white/5 rounded-lg p-3 inline-block">x_(n+1) = g(x_n) = (x_n³ + 1) / 3　　εa = |x_(n+1) − x_n| / x_(n+1) × 100</p>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {rows.map((r) => (
          <div key={r.iter} className="bg-white/5 border border-white/10 rounded-xl p-4">
            ...
            <p>x_n = {r.Tn.toFixed(6)}</p>
            <p>g(x_n) = x_(n+1) = {r.gTn.toFixed(6)}</p>
            <p>f(x_n) = {r.fTn.toFixed(6)}</p>
...
function InterpretationSolution({ raiz, rows }) {
  if (!raiz) return <p>Ajusta x₀ para lograr convergencia (|g'(x)| &lt; 1).</p>
  return (
    <>
      <p>Tras {rows.length - 1} iteraciones, x ≈ {raiz.gTn.toFixed(5)}, εa = {raiz.error?.toFixed(2)}%.</p>
      <p>Esa es una de las tres raíces reales de f(x) = x³ − 3x + 1 (las otras dos, ≈ −1.87939 y ≈ 1.53209, requieren otro x₀ o otra g(x) para converger).</p>
      <div>
        <h3 className="text-fuchsia-400 font-semibold mb-1">Verificación</h3>
        <p>Se sustituye x≈{raiz.gTn.toFixed(5)} en la función original f(x) = x³ − 3x + 1:</p>
        <p>f({raiz.gTn.toFixed(5)}) = {f(raiz.gTn).toFixed(6)} ≈ 0 → confirma que es raíz.</p>
      </div>
    </>
  )
}
...
export default function Examen_P4_PuntoFijo_Cubica() {
  const [T0Str, setT0Str] = useState('0.5')
  const [tolStr, setTolStr] = useState('0.01')
  const T0 = T0Str === '' ? NaN : Number(T0Str)
  const tol = tolStr === '' ? NaN : Number(tolStr)

  const inputError = (T0Str === '' || tolStr === '') ? 'Completa x₀ y la tolerancia.'
    : Number.isNaN(T0) || Number.isNaN(tol) ? 'Valores inválidos.'
    : Math.abs(gPrime(T0)) >= 1 ? `|g'(x₀)|=${Math.abs(gPrime(T0)).toFixed(3)} ≥ 1: no converge. Prueba x₀ cerca de 0.3473 (por ejemplo, entre -0.9 y 0.9).`
    : null
  ...
  return (
    <div className="relative text-slate-100 p-6 md:p-10">
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="max-w-[1400px] mx-auto relative z-10">
        <h1 className="text-3xl md:text-4xl font-bold bg-gradient-to-r from-cyan-400 to-blue-500 bg-clip-text text-transparent">Punto Fijo — f(x) = x³ − 3x + 1</h1>
        <p className="text-slate-400 mt-2">Examen: se propone g(x) = (x³+1)/3 y se demuestra convergencia cerca de la raíz</p>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-6 backdrop-blur-md bg-white/5 border border-white/10 rounded-2xl p-6">
          <div className="flex justify-between items-start">
            <p className="text-slate-300">x = g(x) = (x³ + 1) / 3</p>
            <SolutionButton onClick={() => setOpenSection('context')} />
          </div>
          <p className="text-sm text-slate-500 mt-2">{Number.isNaN(T0) ? 'Completa x₀.' : `x₀ = ${T0} — g'(x₀) = ${gPrime(T0).toFixed(4)}`}</p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-6 backdrop-blur-md bg-white/5 border border-white/10 rounded-2xl p-6 overflow-x-auto">
          ...
          <table className="w-full text-sm">
            <thead><tr className="text-slate-400 border-b border-white/10">{['Iter.', 'x_n', 'g(x_n)', 'f(x_n)', 'εa (%)', 'Cumple'].map(h => <th className="text-left py-2 px-2" key={h}>{h}</th>)}</tr></thead>
            <tbody>
              <AnimatePresence>
                {rows.map((r) => (
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mt-6 mb-10 backdrop-blur-md bg-gradient-to-r from-emerald-500/10 to-cyan-500/10 border border-emerald-400/20 rounded-2xl p-6">
          ...
          {raiz ? (
            <p className="text-slate-300">Tras {rows.length - 1} iteraciones, x ≈ <span className="font-bold text-emerald-300">{raiz.gTn.toFixed(5)}</span>, εa = {raiz.error?.toFixed(2)}%. Es una raíz real de f(x) = x³ − 3x + 1, confirmada porque |g'(x₀)| &lt; 1 garantizó la convergencia desde el inicio.</p>
