...
function f(t) { return 2 * Math.cos(t) - t / 2 }
...
function ContextSolution({ a, b }) {
  ...
  return (
    <>
      <div><h3 className="text-fuchsia-400 font-semibold mb-1">Dato</h3><p>f(x) = 2·cos(x) − x/2 = 0</p></div>
      <div>
...
function GraphSolution({ raiz, a, b }) {
  return (
    <>
      ...
      <div><h3 className="text-fuchsia-400 font-semibold mb-1">Lectura</h3><p>{raiz ? `Raíz aproximada en x ≈ ${raiz.xr.toFixed(5)}.` : 'No converge: revisa que f(a) y f(b) tengan signos opuestos.'}</p></div>
      <div>
        <h3 className="text-fuchsia-400 font-semibold mb-1">Verificación en GeoGebra</h3>
        <p className="text-cyan-300 pl-4">f(x) = Función(2*cos(x) - x/2, {a}, {b})</p>
      </div>
    </>
  )
}
...
function InterpretationSolution({ raiz, rows }) {
  ...
  return (
    <>
      <p>Tras {rows.length} iteraciones, x ≈ {raiz.xr.toFixed(5)}, error {raiz.error?.toFixed(5)}%.</p>
      <p>Esta es la intersección entre y = 2·cos(x) y y = x/2 más cercana al intervalo [1, 2].</p>
    </>
  )
}
...
export default function Examen_P5_FalsaPosicion_Coseno() {
  ...
  const noRootError = !domainError && f(a) * f(b) > 0
    ? 'f(a) y f(b) tienen el mismo signo: no hay garantía de raíz aquí. Hay una raíz cerca de x ≈ 1.2523 — prueba, por ejemplo, [1, 2].'
    : null
  ...
  const chartData = useMemo(() => {
    const safeA = Number.isNaN(a) ? 1 : a
    const safeB = Number.isNaN(b) ? 2 : b
  ...
  return (
    <div className="relative text-slate-100 p-6 md:p-10">
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="max-w-[1400px] mx-auto relative z-10">
        <h1 className="text-3xl md:text-4xl font-bold bg-gradient-to-r from-cyan-400 to-blue-500 bg-clip-text text-transparent">Falsa Posición — f(x) = 2cos(x) − x/2</h1>
        <p className="text-slate-400 mt-2">Examen: tabla de iteraciones hasta k = 4, intervalo inicial [1, 2]</p>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-6 backdrop-blur-md bg-white/5 border border-white/10 rounded-2xl p-6">
          <div className="flex justify-between items-start">
            <p className="text-slate-300">f(x) = 2·cos(x) − x/2 = 0</p>
              ...
              <AnimatePresence>
                {rows.map((r) => (
                  <motion.tr key={r.iter} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="border-b border-white/5">
                    ...
                    <td className="py-1.5 px-2">{r.a.toFixed(5)}</td>
                    <td className="py-1.5 px-2">{r.b.toFixed(5)}</td>
                    <td className="py-1.5 px-2 text-cyan-300">{r.xr.toFixed(5)}</td>
                    <td className="py-1.5 px-2">{r.fxr.toFixed(5)}</td>
                    <td className="py-1.5 px-2">{r.fa.toFixed(5)}</td>
                    <td className="py-1.5 px-2">{r.producto.toFixed(6)}</td>
                    <td className="py-1.5 px-2">{r.error === null ? '—' : r.error.toFixed(5)}</td>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mt-6 mb-10 backdrop-blur-md bg-gradient-to-r from-emerald-500/10 to-cyan-500/10 border border-emerald-400/20 rounded-2xl p-6">
          ...
          {raiz ? (
            <p className="text-slate-300">Tras {rows.length} iteraciones, x ≈ <span className="font-bold text-emerald-300">{raiz.xr.toFixed(5)}</span>, error {raiz.error?.toFixed(5)}%. Es la raíz de f(x) = 2cos(x) − x/2 más cercana al intervalo inicial [1, 2].</p>
